...
MAIN_DISPLAY_SPACES="$(yabai -m query --displays | jq -r '.[] | select(.id == 1) | .spaces | @sh')"
...
for space in ${MAIN_DISPLAY_SPACES}
do
  echo "yabai -m space $space --padding abs:5:5:5:5"
  yabai -m space $space --padding abs:5:5:5:5
done
